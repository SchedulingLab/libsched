#ifndef SCHED_GRAPH_H
#define SCHED_GRAPH_H
...
namespace sched {
  ...
  public:
    ...
    using InEdgeRange = std::ranges::ref_view<const std::set<EdgeId>>;

    InEdgeRange in_edges(VertexId v) const;

    // out edges

    using OutEdgeRange = std::ranges::ref_view<const std::set<EdgeId>>;
