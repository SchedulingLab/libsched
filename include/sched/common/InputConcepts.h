...
namespace sched {
  ...
  template<typename I>
  concept InputConcept = std::regular<I> && requires(I input, std::size_t index) {
  ...
  namespace details {
    ...
    template<typename T>
    concept ReferenceToInput = std::is_reference_v<T> && InputConcept<std::remove_reference_t<T>>;
